...
def encode(to_encode, key=''):
	...
	for code in cid:
		a = random.choice(numbers); b = random.choice(numbers)
		c = random.choice(numbers); d = random.choice(numbers)
		coded_list1.append((((((code+a)*b)+c)*d) ,f"{a}{b}{c}{d}"))
		sg.one_line_progress_meter('DEncoderDEncoder', progress, len(cid), 'key','Encoding...')
		progress += 1
...
def decode(to_decode, key):
	...
	parsed_abcds = [[number for number in abcd] for abcd in abcds]; equivalences = []
	for index in range(len(coded_chars)):
		coded_char = int(coded_chars[index]); a, b, c, d = [int(number) for number in parsed_abcds[index]]
		equivalences.append(int((((coded_char/d)-c)/b)-a))
